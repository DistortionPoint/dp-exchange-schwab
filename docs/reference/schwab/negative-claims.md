...
| No sandbox | prose, published 2025-10-30; re-checked 2026-09-08 | **holds as of the document's own date; UNCONFIRMED since.** "Later this year" meant 2025, and it is now 2026-09-08 with no newer capture in this repo — the portal still answers anonymous reads with `403` (confirmed again this date), so this cannot be re-verified without a signed-in session. A web search for outside confirmation found only a third-party GitHub *mock* server named "Schwab-API-Sandbox" (explicitly not Schwab's own) and one unsourced article claim — neither is the vendor, which is the exact trap this file's own streaming-API story is about. **Not flipped on that evidence.** See `spec-facts.md` §5 for the full account; the next person with portal access should check the Trader API product's own Documentation tab before trusting either the stale promise or the stray search result |
